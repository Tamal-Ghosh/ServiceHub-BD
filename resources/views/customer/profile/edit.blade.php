...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-black text-white">Profile Management</h2>
            <p class="text-sm text-slate-400">View and update your personal account settings</p>
        </div>
        <a href="{{ route('customer.dashboard') }}" class="px-4 py-2 rounded-xl bg-white/[0.05] hover:bg-white/[0.08] border border-white/5 text-slate-300 hover:text-white text-xs font-semibold transition-all">
            Back to Dashboard
        </a>
    </div>
    ...
    {{-- Edit Form --}}
    <form method="POST" action="{{ route('customer.profile.update') }}" class="backdrop-blur-xl bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 md:p-8 space-y-6 shadow-xl">
        @csrf
        @method('PUT')

        {{-- Basic Information Section --}}
        <div>
            <h3 class="text-base font-bold text-white border-b border-white/5 pb-2 mb-4">Account Information</h3>
