...
<div class="max-w-3xl mx-auto space-y-8">
    ...
    <div class="relative overflow-hidden backdrop-blur-xl bg-white/[0.03] border border-white/[0.06] rounded-3xl shadow-2xl shadow-black/20">
        <div class="h-32 bg-gradient-to-br from-rose-600 via-orange-500 to-amber-500 relative">
        ...
        <div class="px-6 md:px-8 pb-6 -mt-14 relative">
            <div class="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                <div class="w-24 h-24 rounded-2xl bg-gradient-to-br from-rose-500 to-amber-500 flex items-center justify-center text-3xl font-black text-white shadow-xl shadow-rose-500/30 border-4 border-slate-950 shrink-0">
                    {{ $user->initials }}
                </div>
                <div class="flex-1 pt-2 sm:pb-1">
                    ...
                    <div class="flex flex-wrap items-center gap-3 mt-1.5">
                        <span class="inline-flex items-center gap-1.5 text-sm text-slate-400">
                            <svg class="w-3.5 h-3.5 text-rose-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            {{ $user->email }}
                        </span>
                        @if($user->phone)
                        <span class="inline-flex items-center gap-1.5 text-sm text-slate-400">
                        ...
                        @endif
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/20">
                            Administrator
                        </span>
                    </div>
                </div>
    ...
    {{-- Edit Form --}}
    <form method="POST" action="{{ route('admin.profile.update') }}" id="profileForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="backdrop-blur-xl bg-white/[0.03] border border-white/[0.06] rounded-2xl overflow-hidden shadow-xl">
            <div class="px-6 md:px-8 py-5 border-b border-white/[0.06] flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-white">Admin Account Details</h3>
                        <p class="text-xs text-slate-500">Your administrator credentials</p>
                    </div>
                </div>
                <button type="button" id="editBtn" onclick="toggleEdit()"
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 text-xs font-semibold transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required disabled
                            class="profile-field w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all disabled:opacity-60 disabled:cursor-not-allowed">
                    ...
                    <div>
                        ...
                        <select name="city" id="city" required disabled
                            class="profile-field w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all appearance-none disabled:opacity-60 disabled:cursor-not-allowed"
        ...
        {{-- Security Card (hidden by default) --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="password" name="password" id="password" placeholder="Minimum 6 characters" minlength="6"
                            class="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all">
        ...
        <div id="actionButtons" class="flex justify-end gap-3 hidden">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-white bg-gradient-to-r from-rose-600 to-amber-500 hover:from-rose-500 hover:to-amber-400 text-sm font-bold shadow-lg shadow-rose-600/20 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-rose-600/30">
